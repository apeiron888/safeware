...
const SupervisorDashboard: React.FC = () => {
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            try {
                if (editingItem) {
                    await api.put(`/supervisor/item/update/${editingItem.id}`, values);
                    toast.success('Item updated successfully');
                } else {
                    await api.post('/supervisor/item/add', values);
    ...
    const confirmDelete = async () => {
        ...
        setIsDeleting(true);
        try {
            await api.delete(`/supervisor/item/remove/${deletingItem.id}`);
    ...
    return (
        <div>
            ...
            <div className="flex justify-between items-center mb-6">
                <h1 className="text-2xl font-semibold text-gray-900 dark:text-white">Supervisor Dashboard - Warehouse Items</h1>
                <button
