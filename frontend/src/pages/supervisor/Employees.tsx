...
import { HiUser, HiUserGroup, HiPencil, HiTrash } from 'react-icons/hi';
...
const SupervisorEmployees: React.FC = () => {
    ...
    const fetchEmployees = async () => {
        try {
            const res = await api.get('/supervisor/employees');
        ...
        onSubmit: async (values) => {
            try {
                ...
                const endpoint = editingEmployee
                    ? `/supervisor/${role.toLowerCase()}/update/${editingEmployee.id}`
                    : `/supervisor/${role.toLowerCase()}/create`;
                ...
                if (editingEmployee) {
                    ...
                    setEmployees(prev => prev.map(emp =>
                        emp.id === editingEmployee.id
                            ? { ...emp, full_name: values.full_name, email: values.email }
                            : emp
                    ));
                } else {
    ...
    const handleEditEmployee = (employee: Employee) => {
        ...
        setSelectedRole(employee.role as 'Supervisor' | 'Staff');
        formik.setValues({
    ...
    const confirmDelete = async () => {
        ...
        setIsDeleting(true);
        try {
            const endpoint = `/supervisor/${deletingEmployee.role.toLowerCase()}/delete/${deletingEmployee.id}`;
    ...
    return (
        <div>
            <div className="flex justify-between items-center mb-6">
                <h1 className="text-2xl font-semibold text-gray-900 dark:text-white">Warehouse Employees</h1>
                <button
